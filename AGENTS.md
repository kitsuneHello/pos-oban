...
- **Customer board (`/callout-customer.html`) is public and login-free.** It reads `GET /board/state` (JSON snapshot) and `GET /board/stream` (SSE: snapshot on connect + on every mutation via `emit()` + 60s heartbeat). `store.getCalloutState()` returns only `{orgName, ready:[order_number], cooking:[order_number]}` (READY/PREPARING) — never products/prices/user data. Keep it minimal when changing.
